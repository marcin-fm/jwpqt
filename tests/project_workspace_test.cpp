...
void roundtrip(const QString& directory) {
  ...
  for (std::size_t active = 0; active < workspace.documents.size(); ++active) {
    ...
    require(!decoded.settings.dictionary.automatic_search && decoded.settings.dictionary.contingent && decoded.settings.dictionary.advanced && decoded.settings.dictionary.require_end &&
