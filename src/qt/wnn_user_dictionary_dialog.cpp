...
WnnUserDictionaryDialog::WnnUserDictionaryDialog(
  ...
  information_action_->setObjectName(QStringLiteral("wnnUserCharacterInformation"));
  radical_action_->setObjectName(QStringLiteral("wnnUserRadicalLookup"));
...
void WnnUserDictionaryDialog::set_lookup_handlers(
...
WnnUserDictionaryDialog::selected_character() const {
  const auto index = selected_index();
  if (!index || editor_model_.entries()[*index].reading.empty()) {
    return std::nullopt;
  }
  return editor_model_.entries()[*index].reading.front();
}

void WnnUserDictionaryDialog::show_information() {
...
void WnnUserDictionaryDialog::show_radical_lookup() {
  const auto code = selected_character();
...
void WnnUserDictionaryDialog::update_actions() {
  ...
  information_action_->setEnabled(selected && static_cast<bool>(information_handler_));
  radical_action_->setEnabled(selected && static_cast<bool>(radical_handler_));
}
